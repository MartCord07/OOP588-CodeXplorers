...
public class MetodosDoctor {
    ...
    public List<Doctor> ListarMedicamentos() {
        List<Doctor> listaMedicamentos = new ArrayList<>();
        FindIterable<Document> documentoMedicinas;
        try {
            documentoMedicinas = coleccionMedicina.find();
            for (Document temp : documentoMedicinas) {
                ...
                medicina.setTipoMed(temp.getString("tipoMed"));// Agregar paciente a la lista
            }
        } catch (MongoException ex) {
            JOptionPane.showMessageDialog(null, "Error al consultar datos" + ex.getMessage());
        } finally {
